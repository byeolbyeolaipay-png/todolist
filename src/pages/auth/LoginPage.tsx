...
export default function LoginPage() {
  ...
  const tryMigrate = async () => {
    const { data } = await import('@/lib/db/supabase').then(m => m.supabase.auth.getSession());
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
        ...
        <p className="text-sm text-gray-500 mb-6">계정이 없으신가요? <Link to="/signup" className="text-blue-500 hover:underline">회원가입</Link></p>
        ...
        <button onClick={handleGoogle}
          className="w-full border border-gray-200 hover:bg-gray-50 rounded-lg py-2 text-sm font-medium text-gray-700 flex items-center justify-center gap-2">
